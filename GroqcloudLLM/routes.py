...
async def extract_clean_text_llam3_3b(
    ...
    try:
        ...
        original_provider = parser.use_ollama
        if provider and provider.lower() in ["groq", "ollama"]:
            temp_parser = ResumeParser(llm_provider=provider.lower())
        else:
            temp_parser = parser
        ...
        print(cleaned_text)
        resume_parser = temp_parser.process_resume(cleaned_text)
        print(resume_parser)
